...
def soloucionar_reto_grupal(request,pk):
	"""esta vista nos permite solucionar el reto grupal """
	lista = [] #esta lista se utiliza  para  los  aprendices  qeu no estan asociado a un reto
	retos = Reto.objects.all()
	p = Persona.objects.get(user=request.user)
	aprendices = Persona.objects.filter(user__is_active=True,user__is_staff=False)#todos los  aprendices
	#aprendices = Persona.objects.exclude(user__is_active=False,user__is_staff=True, user = request.user)#todos los  aprendices
	reto = Reto.objects.get(id=pk) #me saca el id de el reto#
	
	try:
		ya_tengo = Persona_Reto.objects.filter(reto=reto, usuario = request)
	except:
		ya_tengo = None
	print ("========================")
	print (aprendices)
	...
	print ("========================")	
	for a in aprendices:
		lista2=Persona_Reto.objects.filter(reto=reto, persona=a.id)
		if(len(lista2)==0):
			if (p!=a):
				lista.append(a)
		else:
	...
	if request.method == 'POST' :
		form = solucion_reto_grupal(request.POST , request.FILES)
		#if form.is_valid():
		x =  request.POST.getlist('ap')
		print ("xxxxxxxxxxxxxxxx")
		print (map (int, x))
		print (x)
		print ("xxxxxxxxxxxxxxxx")
		for t in x:
			sol = Persona_Reto()#form.save(commit=False)
			sol.reto = reto
			sol.persona = Persona.objects.get(pk=t)
			sol.estado = "sin calificar"
			sol.save()
		if ya_tengo == None:
			yo = Persona_Reto()
			yo.reto = reto
			yo.persona = p
			yo.estado = "sin calificar"
			yo.save()
		return redirect('lista_reto')
				#return redirect('lista_reto')
	else:
		form = solucion_reto_grupal()
		return render(request, 'retos/solucionar_reto_grupal.html', locals())
	return render(request, 'retos/solucionar_reto_grupal.html', locals())
